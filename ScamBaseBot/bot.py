            ...
            if telegram_api is not None and telegram_api.is_connected:
                try:
                    ...
                    if user_info:
                        ...
                        logger.info(f"Получен из Telegram API по ID {user_id}: username={username}")
            ...
            if telegram_api is not None and telegram_api.is_connected:
                try:
                    ...
                    if user_info:
                        ...
                        username = user_info['username'] or f"id{user_id}"
                        logger.info(f"Получен из Telegram API по username {clean_identifier}: id={user_id}, username={username}")
                        return user_id, username
                except Exception as e:
    ...
    try:
        ...
        if real_user_id:
            ...
            if real_username:
                ...
                display_username = f"id{real_user_id}"
        else:
...
async def remove_scammer_dialog(query, user_id: int, scammer_id: str):
    ...
    scammer_info = db.check_user(scammer_id)
    if not scammer_info:
        await query.answer("❌ Скамер не найден в базе!", show_alert=True)
        return
...
async def main():
    ...
    try:
        ...
        # ... (весь ваш код инициализации) ...
        
        print("📡 Ожидание команд...")
        print("Для остановки нажмите Ctrl+C")
        
        # УБИРАЕМ application.run_polling и используем ручное управление
        # Вместо этого создаем и запускаем updater вручную
        ...
        if updater:
            ...
            try:
                ...
                print("\n🛑 Получен KeyboardInterrupt, останавливаюсь...")
            finally:
            ...
            print("❌ Не удалось получить updater")
            await application.stop()
            await application.shutdown()
...
if __name__ == '__main__':
    try:
        # Простой запуск через asyncio.run() - сработает если нет запущенного loop
        asyncio.run(main())
    except RuntimeError as e:
        if "already running" in str(e):
            print("⚠️ Event loop уже запущен, используем альтернативный запуск...")
            # Пытаемся получить существующий loop
            try:
                loop = asyncio.get_event_loop()
                if loop.is_running():
                    print("✅ Используем существующий event loop")
                    # Запускаем main как корутину в существующем loop
                    ...
                    print("✅ Event loop существует но не запущен, запускаем...")
                    loop.run_until_complete(main())
            except Exception as e2:
                print(f"❌ Ошибка альтернативного запуска: {e2}")
                sys.exit(1)
        else:
